...
// number of sections that the gradient is divided to
...
void main() {
	if(gradient_sections > 0.0) {
		...
		// Clamp section to prevent out-of-bounds access
		int nextSection = min(section + 1, 7);
		FragColor = mix(gradient_color[section], gradient_color[nextSection], off);
	} else {
